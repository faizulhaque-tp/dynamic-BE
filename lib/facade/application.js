...
function activateDeactivateApp(reqData, appId) {
  ...
  return validator.validatePayload([
    ...
    .then(function () {
      return App.findOneAndUpdate({ _id: payload._id },  {$set:{isActive: payload.isActive}});
    });
}

function deleteApp(reqData, appId) {
    return App.findOneAndUpdate({ _id: payload._id }, {$set:{isActive: false, isDeleted: true}})
}
