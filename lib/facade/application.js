...
function getAppListing(reqData) {
  return App.find({userId: reqData.userId})
...
function updateApp(reqData, appId) {
  ...
  return validator.validatePayload([
    ...
    .then(function () {
      return App.update({ _id: payload._id, userId: payload.userId }, {$set:payload})
    });
}
...
function activateDeactivateApp(reqData, appId) {
  ...
  return validator.validatePayload([
    ...
    .then(function () {
      return App.update({ _id: payload._id, userId: payload.userId },  {$set:{isActive: payload.isActive}});
    });
}
...
function deleteApp(reqData, appId) {
  ...
  return validator.validatePayload([
    ...
    .then(function () {
      return App.update({ _id: payload._id, userId: payload.userId }, {isActive: false, isDeleted: true})
    });
}
