  ...
  return tokenLookUp.findOneAndRemove({userId: userId, accessToken: token})
    .then(function (object) {
      if (object) {
        ...
        throw new errorHandler.unAuthenticatedError(errorHandler.messages.USER_NOT_FOUND);
      }
    });
}
...
getOrCreateToken.prototype.retrieveToken = Promise.coroutine(function* (username, password) {
  ...
  if (!authenticatedUser) throw new errorHandler.unAuthenticatedError(errorHandler.messages.USER_NOT_REGISTERED);
